CARD_STRENGTHS = ['\0', 'J', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A']
...
def hand_strength(hand : Hand) : Int32
  ...
  if jokers > 0 && jokers < 5
    ...
    raise "Joker's can't be nil!" if jokers.nil?

    max = 
      counts.each.reduce({'\0', 0}) do |accum, kv|
        accum = {'\0', 0} if accum.nil?

        max_card, max_count = accum
        card, count = kv
        next {max_card, max_count} if count < max_count

        {card, count}
      end

    raise "Couldn't find card to sub joker!" if max.nil?

    counts[max[0]] = (max[1] + jokers) unless max.nil?
  end
